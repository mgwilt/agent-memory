...
use clap::ArgMatches;
use nestor_api::{SlotDto, SlotValueDto};
    ...
    match matches.get_one::<String>("json-file") {
        Some(path) => {
            ...
            serde_json::from_str(&body)
                .map(Some)
                .map_err(|err| CliError::usage(err.to_string(), "Explore: nestor guide slots"))
        }
        None => Ok(None),
    }
}
    ...
    T: DeserializeOwned,
{
    match matches.get_one::<String>("rules-file") {
        Some(path) => {
            ...
            serde_json::from_str(&body)
                .map(Some)
                .map_err(|err| CliError::usage(err.to_string(), "Explore: nestor rule eval --help"))
        }
        None => Ok(None),
    }
}
