...
pub fn build_cli() -> Command {
    Command::new("nestor")
        .about("Agent-friendly CLI for the Nestor memory API.")
        ...
        .arg(
            Arg::new("api-url")
                .long("api-url")
                .value_name("URL")
                .help("API base URL [env: NESTOR_API_URL]")
                .global(true),
        )
        .arg(
            Arg::new("agent")
                .long("agent")
                .value_name("AGENT_ID")
                .help("Default agent id [env: NESTOR_AGENT_ID]")
                .global(true),
        )
        .arg(
            Arg::new("format")
...
fn serve_command() -> Command {
    Command::new("serve")
        .about("Start the Nestor API server")
    ...
    Command::new("chunk")
        ...
        .subcommand(
            Command::new("put")
                ...
                .arg(json_file_arg())
                .after_help(format!("Endpoint: POST /v1/memory/chunks\n\nExamples:\n  nestor --agent agent-1 chunk put mem-preference --type fact --slot topic=symbol:preference\n\n{SLOT_GUIDE}\nDocs:\n  docs/cli/commands.md#chunk-put")),
        )
        .subcommand(
            Command::new("get")
        ...
        .subcommand(
            Command::new("patch")
                ...
                .arg(json_file_arg())
                .after_help(format!("Endpoint: PATCH /v1/memory/chunks/{{chunk_id}}\n\nExamples:\n  nestor --agent agent-1 chunk patch mem-preference --expected-version 1 --slot verified=bool:true\n\n{SLOT_GUIDE}\nDocs:\n  docs/cli/commands.md#chunk-patch")),
        )
        .subcommand(
            Command::new("delete")
...
mod tests {
    ...
    fn retrieve_help_contains_endpoint_and_examples() {
        let matches = build_cli().try_get_matches_from(["nestor", "retrieve", "--help"]);
