use clap::ArgMatches;
use nestor_api::PracticeRequest;
...
pub async fn run(
    ...
    let request = if let Some(request) = parse_json_file::<PracticeRequest>(matches)? {
        request
    } else {
        PracticeRequest {
            agent_id: options
                .agent_id
                .clone()
                .ok_or_else(|| missing_agent_error("practice"))?,
    ...
    if options.verbose {
        eprintln!("POST {}/v1/memory/practice", client.config().api_url);
    }
    let response = client
