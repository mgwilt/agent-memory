...
use clap::ArgMatches;
use nestor_client::{ApiClientConfig, ErrorCategory, NestorClient};
...
async fn main() {
    ...
    let client = NestorClient::new(config);
...
pub fn missing_agent_error(command: &str) -> CliError {
    CliError::usage(
        ...
        format!("Use: nestor --agent <agent-id> {command}"),
    )
}

pub fn map_client_error(error: nestor_client::ClientError) -> CliError {
    let hint = match error.category() {
        ErrorCategory::Unavailable => {
            "Start API: nestor serve\nCheck config: nestor doctor".to_string()
        }
        ErrorCategory::BadRequest | ErrorCategory::NotFound | ErrorCategory::Conflict => {
            "Explore: nestor guide errors".to_string()
        }
        ErrorCategory::Usage | ErrorCategory::Internal => {
            "Explore: nestor guide commands".to_string()
        }
    };
