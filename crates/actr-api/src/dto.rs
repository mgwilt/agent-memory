...
use actr_core::{Chunk, ChunkId, ChunkType, MemoryError, MemoryResult, Slot, SlotValue};
use actr_rules::{
...
use actr_store::{RetrievalMissReason, RetrievalOutcome, RetrievalStatus};
...
impl ProductionRuleDto {
    pub fn into_rule(self) -> MemoryResult<ProductionRule> {
        ...
        let mut rule = ProductionRule::new(
            actr_rules::RuleId::from(self.rule_id),
            self.name,
            self.conditions
                .into_iter()
...
impl BufferConditionDto {
    pub fn try_into_condition(self) -> MemoryResult<actr_rules::BufferCondition> {
        Ok(actr_rules::BufferCondition {
...
impl From<actr_rules::RuleMatch> for RuleMatchDto {
    fn from(value: actr_rules::RuleMatch) -> Self {
        Self {
...
impl From<actr_rules::RuleCandidateDiagnostic> for RuleCandidateDiagnosticDto {
    fn from(value: actr_rules::RuleCandidateDiagnostic) -> Self {
        Self {
            rule_id: value.rule_id.0,
            name: value.name,
