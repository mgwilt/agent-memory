...
async fn cli_covers_all_api_routes_and_validates_retrieval_math() -> TestResult<()> {
    ...
    assert!(metrics.contains("actr_memory_retrieval_hits_total 2"));
    assert!(metrics.contains("actr_memory_candidates_examined 1"));
...
fn run(api_url: &str, args: &[&str]) -> TestResult<Output> {
    let output = Command::new(env!("CARGO_BIN_EXE_actr-memory"))
        .arg("--api-url")
        .arg(api_url)
        .args(args)
        .output()?;
...
async fn start_api() -> TestResult<String> {
    ...
    tokio::spawn(async move {
        let result = axum::serve(listener, actr_api::app()).await;
