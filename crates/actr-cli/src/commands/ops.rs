...
pub fn manifest(client: &ActrClient, options: &GlobalOptions) -> Result<(), CliError> {
...
pub async fn health(client: &ActrClient, options: &GlobalOptions) -> Result<(), CliError> {
    if options.verbose {
        eprintln!("GET {}/healthz", client.config().api_url);
    }
...
pub async fn ready(client: &ActrClient, options: &GlobalOptions) -> Result<(), CliError> {
    if options.verbose {
        eprintln!("GET {}/readyz", client.config().api_url);
    }
