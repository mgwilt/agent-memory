...
pub fn render_manifest(routes: &[actr_client::HttpRoute]) -> String {
    routes
        .iter()
...
pub fn json_manifest(routes: &[actr_client::HttpRoute]) -> serde_json::Value {
    json!(
        routes
            .iter()
            .map(|route| json!({
...
fn render_slots(slots: &std::collections::BTreeMap<String, actr_api::SlotValueDto>) -> String {
    if slots.is_empty() {
...
fn render_slot_value(value: &actr_api::SlotValueDto) -> String {
    match value {
        actr_api::SlotValueDto::Symbol(value) => format!("symbol:{value}"),
        actr_api::SlotValueDto::Text(value) => format!("text:{value}"),
        actr_api::SlotValueDto::Number(value) => format!("number:{value}"),
        actr_api::SlotValueDto::Bool(value) => format!("bool:{value}"),
    }
}
