...
impl RuntimeProfile {
    pub fn parse(value: &str) -> Result<Self, String> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            _ => Err("ACTR_PROFILE must be one of development, staging, or production".to_string()),
        }
    }
...
impl MemgraphSecurityConfig {
    ...
    pub fn hardened() -> Self {
        Self {
            ...
            credentials: Some(SecretSource::EnvVar("ACTR_MEMGRAPH_PASSWORD".to_string())),
        }
    }
...
impl RuntimeConfig {
    ...
    pub fn from_env_vars(vars: impl IntoIterator<Item = (String, String)>) -> Result<Self, String> {
        ...
        if let Some(tls_enabled) = vars.get("ACTR_MEMGRAPH_TLS_ENABLED") {
            config.memgraph_security.tls_enabled = parse_bool(
                tls_enabled,
                "ACTR_MEMGRAPH_TLS_ENABLED must be true or false",
            )?;
        }
        if let Some(ca_file) = vars.get("ACTR_MEMGRAPH_TLS_CA_FILE") {
            config.memgraph_security.tls_ca_file = empty_as_none(ca_file);
        }
        if let Some(server_name) = vars.get("ACTR_MEMGRAPH_TLS_SERVER_NAME") {
            config.memgraph_security.tls_server_name = empty_as_none(server_name);
        }
        if let Some(password_env) = vars.get("ACTR_MEMGRAPH_PASSWORD_ENV") {
            config.memgraph_security.credentials =
                empty_as_none(password_env).map(SecretSource::EnvVar);
        } else if vars
            .get("ACTR_MEMGRAPH_PASSWORD")
            .is_some_and(|password| !password.trim().is_empty())
        {
            config.memgraph_security.credentials =
                Some(SecretSource::EnvVar("ACTR_MEMGRAPH_PASSWORD".to_string()));
        }
        if let Some(password_file) = vars.get("ACTR_MEMGRAPH_PASSWORD_FILE") {
            config.memgraph_security.credentials =
...
mod tests {
    ...
    fn from_env_vars_parses_profile_and_security_overrides() {
        let config = RuntimeConfig::from_env_vars([
            ("ACTR_PROFILE".to_string(), "prod".to_string()),
            (
                "ACTR_MEMGRAPH_URI".to_string(),
                "bolt+s://memgraph.private:7687".to_string(),
            ),
            (
                "ACTR_MEMGRAPH_PASSWORD_FILE".to_string(),
                "/run/secrets/memgraph-password".to_string(),
            ),
            (
                "ACTR_MEMGRAPH_TLS_CA_FILE".to_string(),
                "/etc/ssl/private/memgraph-ca.pem".to_string(),
            ),
            (
                "ACTR_MEMGRAPH_TLS_SERVER_NAME".to_string(),
                "memgraph.private".to_string(),
            ),
            ("ACTR_CANDIDATE_LIMIT".to_string(), "64".to_string()),
            ("ACTR_RETRIEVAL_THRESHOLD".to_string(), "0.5".to_string()),
        ])
    ...
    fn invalid_profile_and_bounds_are_reported() {
        let profile_error =
            RuntimeConfig::from_env_vars([("ACTR_PROFILE".to_string(), "sandbox".to_string())])
                .expect_err("invalid profile should fail");
        assert!(profile_error.contains("ACTR_PROFILE"));

        let bound_error =
            RuntimeConfig::from_env_vars([("ACTR_CANDIDATE_LIMIT".to_string(), "0".to_string())])
