...
mod tests {
    ...
    fn includes_required_service_metrics() {
        ...
        for name in [
            "actr_memory_retrieve_latency_ms",
            "actr_memory_candidates_examined",
            "actr_memory_activation_compute_ms",
            "actr_memory_retrieval_hits_total",
            "actr_memory_retrieval_misses_total",
            "actr_memory_session_lock_contention_total",
            "actr_memory_write_conflicts_total",
        ] {
    ...
    fn renders_prometheus_text_with_types_and_defaults() {
        ...
        assert!(output.contains("# TYPE actr_memory_retrieval_hits_total counter"));
        assert!(output.contains("actr_memory_retrieval_hits_total 2"));
        assert!(output.contains("# TYPE actr_memory_retrieve_latency_ms gauge"));
        assert!(output.contains("actr_memory_retrieve_latency_ms 3.5"));
        assert!(output.contains("actr_memory_candidates_examined 0"));
    }
    ...
    fn non_finite_samples_are_rendered_as_zero() {
        ...
        assert!(output.contains("actr_memory_activation_compute_ms 0"));
    }
}
