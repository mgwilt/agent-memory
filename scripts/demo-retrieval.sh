...
assert_matches "$tmp_dir/retrieve-response.json" '"chunk_id"[[:space:]]*:[[:space:]]*"ck-demo-nestor"' "Demo retrieval returned the wrong chunk"
...
assert_matches "$metrics" '^nestor_memory_retrieval_hits_total[[:space:]]+[1-9][0-9]*(\.[0-9]+)?$' "Retrieval hit metric was not incremented"
